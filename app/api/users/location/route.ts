...
import { broadcastLocationUpdate } from '@/lib/websocket-manager';
    ...
    const updatedUser = await queryOne(
      `UPDATE users
       SET latitude = $1,
           ...
           updated_at = NOW()
       WHERE id = $3
       RETURNING id, first_name, latitude, longitude, share_location, location_updated_at`,
      [latitude, longitude, payload.id]
    );

    // Broadcast location update to nearby connected WebSocket clients
    if (updatedUser) {
      const latestMood = await queryOne(
        `SELECT vibe, text FROM moods WHERE user_id = $1 AND expires_at > NOW() ORDER BY created_at DESC LIMIT 1`,
        [payload.id]
      );
      broadcastLocationUpdate({
        ...
        vibe: latestMood?.vibe ?? null,
        latest_mood: latestMood?.text ?? null,
      });
    }
